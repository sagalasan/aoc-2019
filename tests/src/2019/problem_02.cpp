...
std::uint64_t run_intcode(std::vector<uint64_t> memory) {
    aoc::year_2019::Intcode intcode(std::move(memory));
    return intcode.run_program();
}

TEST(aoc2019, problem_02_parse_input) {
    ASSERT_EQ(std::vector<std::uint64_t>({1,9,10,3,2,3,11,0,99,30,40,50}), aoc::year_2019::day02::parse_input("1,9,10,3,2,3,11,0,99,30,40,50"));
}
